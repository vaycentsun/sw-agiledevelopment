...
if [ -f "$REPO_ROOT/.opencode/plugins/sw-agiledevelopment.js" ]; then
    echo "  [PASS] Plugin file exists"
else
    echo "  [FAIL] Plugin file not found: .opencode/plugins/sw-agiledevelopment.js"
    FAILED=1
fi

# Test 2: Plugin file is readable
if [ -r "$REPO_ROOT/.opencode/plugins/sw-agiledevelopment.js" ]; then
...
if [ -s "$REPO_ROOT/.opencode/plugins/sw-agiledevelopment.js" ]; then
...
if grep -q "sw-" "$REPO_ROOT/.opencode/plugins/sw-agiledevelopment.js"; then
