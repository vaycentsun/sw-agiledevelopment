...
当你面对**多个**失败时，先判断独立性，再选择 Skill：

| 情况 | 选择 | 理由 |
|------|------|------|
| 只有 1 个失败 | **本技能** | 系统化深度调查 |
| 多个失败，但**相关**（修复一个可能修复其他） | **本技能** | 并行浪费。相关失败应一起调查，避免重复 |
| 多个失败，**独立**（不同子系统、不同根因） | **sw-parallel-debugging** | 并行分派 Agent，每个调查一个独立问题 |
| 多个失败，**独立**但有共享状态 | **本技能** | 顺序调查，避免 Agent 干扰 |

**关键判断：** 失败的根因是否可能相同？如果可能相同 → 用本技能一起调查。如果确定不同 → 用 `sw-parallel-debugging`。
...
**相关 Skill**:
- sw-parallel-debugging - 多个**独立**失败时的并行调查入口
- sw-code-review - 审查修复
